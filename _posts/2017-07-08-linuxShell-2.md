...
##### Linux文件系统挂载管理（修改文件系统一定要卸载磁盘）
    ...
    Linux需要手工进行挂载操作或配置系统进行自动挂载。/dev/sda3 ext4  --挂载-->/mnt
...
Linux权限基于UGO模型进行控制：
	...
	4. 修改权限
		...
		递归修改文件夹内的权限chmod -R a-x linuxcast.net
...
课时20.Linux命令行文本处理工具
	...
	基于关键字搜索
		...
		find / -user linuxcast 2> /dev/null | grep Video  ----->所以属于user用户的文件 包含Video关键字 将输出错误信息不显示出来。
			-i 搜索时不区分大小写
			-n 显示结果所在的行数
			-v 输出不带关键字的行
