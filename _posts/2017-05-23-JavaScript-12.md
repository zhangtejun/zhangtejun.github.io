...
如果对原型链这部分不是很清楚，可以看《JavaScript之原型到原型链》
...
##### 事件
```
// event：指定事件名； function：指定要事件触发时执行的函数；useCapture：布尔值，指定事件是否在捕获或冒泡阶段执行。
//（true- 事件句柄在捕获阶段执行。默认false）
// addEventListener() 方法为元素附加事件处理程序而不会覆盖已有的事件处理程序。(添加几次 就会有几个事件)
